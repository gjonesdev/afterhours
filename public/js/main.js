		...
		reportFormDocument.addEventListener("submit", (event) => {
			...
			if (firstNameInput.value.trim()) {
				...
				if (notNumbersRegex.exec(firstNameInput.value) === null) {
					...
					errorDiv.className = "error";
					errorDiv.innerHTML =
						"Name should not contain numbers, just letters";
					firstNameInput.focus();
					event.preventDefault();
					return;
				}
...
const validateAccountType = (accountTypeInput) => {
	...
	if (
		!(
			accountTypeInput.value === "patron" ||
			accountTypeInput.value === "owner"
		)
	) {
...
$("#filterForm").on("submit", (e) => {
	...
	$.ajax({
		...
		(res) => $("#barList").append("<li>No bars matching the selected tags.</li>")
	);
});
