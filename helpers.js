...
export const validateDate = (eventDate, startTime) => {
  if (date.isValid(eventDate, "YYYY-MM-DD")) {
    eventDate = date.transform(eventDate, "YYYY-MM-DD", "MM/DD/YYYY");
  }
  if (!date.isValid(eventDate, "MM/DD/YYYY")) throw "Invalid event date!";
  const now = new Date();
  const dateObj = date.parse(eventDate + " " + startTime, "MM/DD/YYYY hh:mm A");
  if (now > dateObj) throw "Event date needs to be a future date!";
  return eventDate;
};
//validating time
export const validateTime = (time, type) => {
  if (date.isValid(time, "HH:mm")) {
